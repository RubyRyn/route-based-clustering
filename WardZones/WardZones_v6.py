...
class WardZoneAssignment:
    ...
    def _select_seeds(self) -> List[str]:
        """
        Pick K seed wards by random selection with minimum spacing constraint.
        Each new seed must be at least min_seed_spacing km from all existing seeds.
        """
        ...
        # Compute min seed spacing from the data
        # -------------------------------------------
        # max_pairwise = 0
        # for i, pcode_a in enumerate(candidate_wards):
        #     clients_a = self.ward_clients.get(pcode_a, [])
        #     if not clients_a:
        #         continue
        #     for pcode_b in candidate_wards[i + 1:]:
        #         clients_b = self.ward_clients.get(pcode_b, [])
        #         if not clients_b:
        #             continue
        #         # Use min road distance between any client pair
        #         # (closest points of the two wards)
        #         min_dist = min(
        #             self.road_matrix[ca + 1, cb + 1]
        #             for ca in clients_a for cb in clients_b
        #         )
        #         if min_dist > max_pairwise:
        #             max_pairwise = min_dist
        # min_seed_spacing = max_pairwise / np.sqrt(self.n_zones)
        # print(f"  Max pairwise ward distance: {max_pairwise:.1f} km")
        # print(f"  Min seed spacing: {min_seed_spacing:.1f} km")
        # ----------------------------------------------------
        # office_dists = [self.ward_office_dist[p] for p in candidate_wards]
        # min_seed_spacing = (np.mean(office_dists) + np.std(office_dists)) / np.sqrt(self.n_zones)
        # print(f"  Office distance mean: {np.mean(office_dists):.1f} km, std: {np.std(office_dists):.1f} km")
        # print(f"  Min seed spacing: {min_seed_spacing:.1f} km")
        # ----------------------------------------------------
        # Use actual client distances, not ward averages
        all_client_dists = [self.road_matrix[0, idx + 1] 
                            for idx in range(len(self.clients))]
        # min_seed_spacing = (np.mean(all_client_dists) + np.std(all_client_dists)) / np.sqrt(self.n_zones)
        min_seed_spacing = np.mean(all_client_dists)
        # min_seed_spacing = np.std(all_client_dists)
        print(f"  Min seed spacing (mean office distance): {min_seed_spacing:.1f} km")
        # ----------------------------------------------------

        def ward_road_distance(pcode_a, pcode_b):
            clients_a = self.ward_clients.get(pcode_a, [])
            clients_b = self.ward_clients.get(pcode_b, [])
        ...
        shuffled = candidate_wards.copy()
        np.random.shuffle(shuffled)

        seeds = []
        seed_set = set()

        for pcode in shuffled:
            if len(seeds) >= self.n_zones:
                break

            # Check spacing against all existing seeds
            too_close = False
            for existing_seed in seeds:
                dist = ward_road_distance(pcode, existing_seed)
        ...
        # If we didn't get enough seeds, relax and fill remaining
        if len(seeds) < self.n_zones:
            print(f"    Got {len(seeds)} seeds with spacing constraint, "
                f"filling remaining {self.n_zones - len(seeds)} without constraint...")
            for pcode in shuffled:
                if len(seeds) >= self.n_zones:
                    break
                if pcode in seed_set:
                    continue
                seeds.append(pcode)
                seed_set.add(pcode)
        ...
        for i in range(len(seeds)):
            for j in range(i + 1, len(seeds)):
                clients_a = self.ward_clients.get(seeds[i], [])
                clients_b = self.ward_clients.get(seeds[j], [])
                min_dist = float('inf')
                min_ca, min_cb = None, None
                for ca in clients_a:
                    for cb in clients_b:
                        d = self.road_matrix[ca + 1, cb + 1]
                        if d < min_dist:
                            min_dist = d
                            min_ca = ca
                            min_cb = cb
                print(f"    Seed {i+1} ↔ Seed {j+1}: {min_dist:.1f} km "
                    f"(client {min_ca} ↔ client {min_cb})")

        print(f"\nPhase 2 (Seed): Selected {len(seeds)} seed wards "
            f"(random with {min_seed_spacing:.1f} km spacing)")
        return seeds
    ...
    def _grow(self, seeds: List[str]):
        ...
        zone_capped = {}  # Not used — no capping, Phase 4 handles balance
