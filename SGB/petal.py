...
Inspired by postal code zoning: divide the delivery area into 
petal-shaped slices radiating from the office/farm.

Phase 1 (Slice)  — Compute bearing from office to each client,
                    sort by angle, cut into K slices with roughly
                    equal client counts (density-aware widths)
Phase 2 (Swap)   — Fix road-distance violations: if a client's
                    median road distance to another cluster is lower
                    than to its own cluster, swap it (respecting size limits)

Each petal contains a mix of near and far clients, creating natural
delivery routes that go out from the office and come back.
"""
...
class PetalClustering:
    ...
    balanced by client density, refined by road distance.
    """
    ...
    def __init__(self, road_matrix: np.ndarray, locations: List[Location],
        ...
        self.petal_boundaries = None  # Angular boundaries of each petal
        self.history = []

        print(f"PetalClustering initialized:")
    ...
    # ══════════════════════════════════════════════
    # PHASE 1: SLICE — Angular partitioning
    # ══════════════════════════════════════════════

    def _slice(self) -> np.ndarray:
        """
        Sort clients by bearing from office, then cut into K slices
        with roughly equal client counts.

        Dense directions get narrower slices, sparse directions get wider ones.
        """
        ...
        # Divide sorted list into K roughly equal chunks
        chunk_size = self.n_clients / self.n_clusters
        boundaries = []
        ...
        for k in range(self.n_clusters):
            ...
            # Record angular boundary
            start_bearing = self.bearings[sorted_indices[start]]
            end_bearing = self.bearings[sorted_indices[end - 1]]
            boundaries.append((start_bearing, end_bearing))

        self.petal_boundaries = boundaries

        sizes = self._get_cluster_sizes(labels)
        print(f"\nPhase 1 (Slice): Angular partitioning from office")
    ...
    def _find_violations(self, labels: np.ndarray) -> List[Tuple[int, int, int, float]]:
        """
        Find clients that are closer (by median road distance) to another
        cluster than to their own.
        ...
        for client_idx in range(self.n_clients):
            ...
            # Check neighboring petals only (adjacent in angular order)
            # This prevents swapping to a petal on the opposite side of the office
            neighbors = self._get_adjacent_petals(current_cluster)

            best_alt_cluster = None
            best_alt_median = own_median  # Must beat current
    ...
    def _get_adjacent_petals(self, petal_id: int) -> List[int]:
        """
        Get the two petals adjacent to this one in angular order.
        Wraps around (petal 0 is adjacent to petal K-1).
        """
        prev_petal = (petal_id - 1) % self.n_clusters
        next_petal = (petal_id + 1) % self.n_clusters
        return [prev_petal, next_petal]

    def _swap(self, labels: np.ndarray) -> np.ndarray:
        """
        Road-distance-based swap refinement.
        Only swaps between adjacent petals to maintain petal structure.
    ...
    def _draw_petals(self, ax, labels, colors, show_violations=True):
        ...
        # Draw petal wedge lines from office to max distance in each petal
        for cluster_id in range(self.n_clusters):
            ...
            # Draw lines from office to the furthest client in each petal
            # to show the petal direction
            dists = np.sqrt((cluster_lons - office_lon)**2 +
                           (cluster_lats - office_lat)**2)
            furthest = np.argmax(dists)
            ax.plot(
                [office_lon, cluster_lons[furthest]],
                [office_lat, cluster_lats[furthest]],
                color=colors[cluster_id], alpha=0.3, linewidth=1,
                linestyle='--', zorder=1
            )

        # Draw petal boundary lines
        sorted_indices = np.argsort(self.bearings)
        chunk_size = self.n_clients / self.n_clusters
        for k in range(self.n_clusters):
            start = int(round(k * chunk_size))
            if start < len(sorted_indices):
                boundary_client = sorted_indices[start]
                ax.plot(
                    [office_lon, lons[boundary_client]],
                    [office_lat, lats[boundary_client]],
    ...
    def visualize_petals(self, filename: str = "Output/petal_final.png"):
        ...
        sizes = self._get_cluster_sizes(self.labels)
        ax.set_title(
            f"Petal Clustering | K={self.n_clusters}\n"
            f"Sizes: [{sizes.min()}-{sizes.max()}] | "
            f"Std: {sizes.std():.1f} | "
    ...
    def visualize_comparison(self, filename: str = "Output/petal_comparison.png"):
    ...
    # ══════════════════════════════════════════════
    # Polar visualization (bird's eye view of petals)
    # ══════════════════════════════════════════════

    def visualize_polar(self, filename: str = "Output/petal_polar.png"):
        """
        Polar plot showing petals as angular slices.
        Radius = road distance from office, angle = bearing.
        """
        if self.labels is None:
            print("Run fit() first.")
            return
    ...
    def assign_new_client(self, new_location: Location,
        ...
        # Find which petal this bearing falls into
        # Score each petal: combination of angular proximity and road distance
        sizes = self._get_cluster_sizes(self.labels)
        best_cluster = None
        best_score = np.inf
        ...
        for cluster_id in range(self.n_clusters):
            ...
            # Combined score (angular + road, weighted)
